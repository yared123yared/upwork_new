...
class PaymentModelListBloc
    extends Bloc<PaymentModelListEvent, PaymentModelListState> {
  PaymentModelRepository mrepository = PaymentModelRepository();
  PaymentModelListBloc() : super(PaymentModelListState());

  @override
  Stream<PaymentModelListState> mapEventToState(
    PaymentModelListEvent event,
  ) async* {
    ...
    if (event is getListDataWithSearchParameter) {
      yield IsBusy();
      PaymentModelRepositoryReturnData ud =
          await mrepository.getPaymentModelWithOfferingSearch(
              event.entitytype,
              event.entityid,
              event.sessionterm,
