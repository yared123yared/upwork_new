  ...
  Stream<PaymentPeriodInfoState> mapEventToState(
    ...
    if (event is createItem) {
      ...
      PaymentPeriodInfoRepositoryReturnData ud = await mrepository
          .createPaymentPeriodInfo(event.item,event.entitytype, event.entityid);
...
if (event is getForNewEntry) {
      ...
      if (ud.errortype == -1) yield IsReadyForDetailsPage(entitytype:event.entitytype,entityid:event.entityid,detailstype: "start" ,sessionterms: ud.sessionterms );
      else if (ud.errortype == 1)
        yield HasLogicalFaliur(error:ud.error);
      else
        yield HasExceptionFaliur(error:ud.error);
    ...
    if (event is updateItem) {
      yield IsBusy();
      PaymentPeriodInfoRepositoryReturnData ud = await mrepository
          .updatePaymentPeriodInfo(event.item,event.entitytype, event.entityid);
    ...
    if (event is updateItemWithDiff) {
      yield IsBusy();
      PaymentPeriodInfoRepositoryReturnData ud = await mrepository
          .updatePaymentPeriodInfoWithDiff(event.newitem,event.olditem,event.entitytype, event.entityid);
