...
import './vehicle_relation_form.dart';

class VehicleRelationListList extends StatefulWidget {
  final String entityid;
  final String entitytype;
  VehicleRelationListList({this.entitytype, this.entityid});

  @override
  _VehicleRelationListListState createState() =>
      _VehicleRelationListListState();
}

class _VehicleRelationListListState extends State<VehicleRelationListList> {
  listbloc.VehicleRelationListBloc mlistbloc;
  ...
  VoidCallback addButtonActions({
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => VehicleRelationForm(
            vehicleRelation: null,
  ...
  List<ListStateClass> toCommonListState(
      List<Relation> listItems, BuildContext context) {
    List<ListStateClass> _dynamicList = [];
    listItems.asMap().forEach((index, item) {
      _dynamicList.add(ListStateClass(
        title: "${item.name ?? ''} ${item.isActive ?? ""}",
        tapAction: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => VehicleRelationForm(
                  vehicleRelation: item,
                  entitytype: widget.entitytype,
                  entityid: widget.entityid,
                  givenreloadaction: doreload),
            ),
        ...
        deleteAction: () async {
          ...
          if (docancel) {
            BlocProvider.of<listbloc.VehicleRelationListBloc>(context).add(
  ...
  Widget build(BuildContext context) {
    return BlocProvider.value(
      ...
      child: Scaffold(
          appBar: AppBar(
            title: Text("Vehicle Relation List"),
            centerTitle: true,
          ),
          body: BlocListener<listbloc.VehicleRelationListBloc,
              listbloc.VehicleRelationListState>(listener: (context, state) {
          ...
          }, child: BlocBuilder<listbloc.VehicleRelationListBloc,
              listbloc.VehicleRelationListState>(builder: (context, state) {
      ...
      slivers: [
        ...
        SliverToBoxAdapter(
            child: CommonListPage(
                canSearch: false,
                updateAction: null,
                appBarTitle: "Vehicle Relation List",
                dynamicListState: "Vehicle Relation List",
