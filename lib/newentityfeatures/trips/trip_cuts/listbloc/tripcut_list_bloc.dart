...
class TripCutListBloc
    extends Bloc<TripCutListEvent, TripCutListState> {
  TripCutRepository mrepository = TripCutRepository();
  TripCutListBloc() : super(TripCutListState());

  @override
  Stream<TripCutListState> mapEventToState(
    TripCutListEvent event,
  ) async* {
    if (event is getListData) {
      yield IsBusy();
      TripCutRepositoryReturnData ud =
    ...
    if (event is getListDataWithSearchParameter) {
      yield IsBusy();
      TripCutRepositoryReturnData ud =
          await mrepository.getTripCutWithOfferingSearch(
              event.entitytype,
              event.entityid,
              event.sessionterm,
    ...
    if (event is deleteItemWithData) {
      yield IsBusy();
      TripCutRepositoryReturnData ud =
          await mrepository.deleteTripCutWithData(
