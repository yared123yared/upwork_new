...
class HandleRelationRepository {
  ...
  static Future<List<RouteTripModel>> findTrips(
    ...
    final tripCutDoc = await _firestore
        .doc(
            'SERVICEPROVIDERINFO/$serviceID/TRIPCUTS/${tripCutShort.docId}')
        .get();
    return TripCut.fromJson(
            json: tripCutDoc.data(), docId: tripCutDoc.id)
        .routeTrips;
  }
  ...
  static bool _canCheckIn(
    ...
    String enteredDayOfWeek =
        DateFormat('EEEE').format(staffStartDate).substring(0, 3).toLowerCase();
